...
export default function SearchDialog({ open, onOpenChange, categories }: SearchDialogProps) {
  ...
  const { data: results = [], isFetching: loading } = useSearchProducts(query);
        ...
        <CommandList className="flex-1 max-h-[55vh] md:max-h-[480px] p-2 md:p-4 font-poppins scrollbar-hide overflow-y-auto bg-white">
          {query.length > 0 && results.length === 0 && !loading && (
            <CommandEmpty className="py-10 md:py-16 text-center">
              <div className="flex flex-col items-center gap-3 px-4">
                ...
                <Button variant="outline" size="sm" className="mt-2 h-8 rounded-[5px] px-5 border-zinc-200 text-[9px] font-bold uppercase tracking-widest" onClick={() => setQuery('')}>
                  Reset
                </Button>
              </div>
            </CommandEmpty>
          ...
          {results.length > 0 && (
            <CommandGroup heading={
              <span className="flex items-center justify-between text-zinc-400 font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] pb-2 px-2">
                Results <span className="text-primary">{results.length} Found</span>
              </span>
            }>
              <div className="grid grid-cols-1 gap-1 md:gap-2">
                  ...
                  >
                    <div className="flex items-center gap-3 md:gap-4 w-full">
                      <div className="relative h-12 w-12 md:h-16 md:w-16 rounded-[5px] bg-white border border-zinc-100 p-1 overflow-hidden shrink-0">
                        <AppImage src={getImageUrl(product.images)} alt={product.name} fill className="object-contain group-hover:scale-105 transition-transform" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-black text-xs md:text-sm line-clamp-1 group-hover:text-primary transition-colors">{product.name}</h4>
                        <p className="text-[9px] md:text-[10px] text-zinc-400 font-medium uppercase tracking-wider">{product.category}</p>
                      </div>
                      <div className="text-right shrink-0 pr-1 md:pr-2">
                        <p className="font-black text-black text-xs md:text-sm tracking-tight">AED {product.price.toFixed(2)}</p>
                      </div>
                    </div>
                  </CommandItem>
                ))}
                ...
                >
                  Full results for "{query}" <ArrowRight className="h-3 w-3 md:h-4 md:w-4" />
                </button>
              </div>
            </CommandGroup>
          )}
          ...
          {query.length === 0 && (
            <div className="space-y-5 md:space-y-8 py-2">
              <CommandGroup heading={<span className="text-zinc-400 font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] pb-2 px-2">Popular</span>}>
                <div className="flex flex-wrap gap-1.5 px-2">
                  {['iPhone', 'MacBook', 'Samsung', 'Watch'].map((item) => (
                    <button key={item} onClick={() => setQuery(item)} className="px-3 md:px-5 py-1.5 md:py-2.5 rounded-[5px] bg-zinc-50 text-zinc-600 text-[10px] md:text-[11px] font-bold uppercase tracking-wider hover:bg-black hover:text-white transition-all border border-zinc-100">
                      {item}
                    </button>
                  ))}
                </div>
              ...
              <CommandGroup heading={<span className="text-zinc-400 font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] pb-2 px-2">Browse</span>}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 px-2 pb-4">
                  {categories.slice(0, 8).map((category) => (
                    <button key={category.id} onClick={() => handleSelectCategory(category.slug)} className="flex flex-col items-center justify-center py-3 md:py-5 px-2 rounded-[5px] bg-zinc-50 border border-zinc-100 hover:border-primary/50 hover:bg-white transition-all group">
                      <span className="text-[9px] md:text-[11px] font-black text-black uppercase tracking-widest group-hover:text-primary transition-colors text-center">{category.name}</span>
                    </button>
                  ))}
                </div>
              </CommandGroup>
