              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white rounded-[32px] border border-zinc-100 p-8">
                  <h3 className="text-[11px] font-black uppercase tracking-widest text-black mb-6 flex items-center gap-2">
                    <Box className="h-4 w-4 text-primary" /> Order Items
                  </h3>
                  <div className="space-y-4">
                    {orderItems.map((item: any, i: number) => {
                       const currentPrice = item.price || item.product?.price || 0;
                       return (
                        <div key={i} className="flex justify-between items-center bg-zinc-50/50 p-4 rounded-2xl border border-zinc-100/50">
                          <div>
                            <p className="text-sm font-bold text-black">{item.product_name || item.productName || 'Product'}</p>
                            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mt-0.5">Qty: {item.quantity}</p>
                          </div>
                          <span className="text-sm font-black text-black">AED {(Number(currentPrice) * (item.quantity || 1)).toFixed(2)}</span>
                        </div>
                       );
                    })}
                  </div>
                  <div className="mt-6 pt-6 border-t border-zinc-50 flex justify-between items-center">
                    <span className="text-xs font-black uppercase tracking-widest text-zinc-400">Order Total</span>
                    <span className="text-2xl font-black text-black">
                      AED {(
                        orderItems.reduce((sum: number, item: any) => sum + (Number(item.price || item.product?.price || 0) * (item.quantity || 1)), 0) + 
